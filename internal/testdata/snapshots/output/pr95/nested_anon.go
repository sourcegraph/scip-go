...
//     ^^^^^^^^^^^^^ definition 0.1.test `sg/pr95`/ContainerAnon#
//     documentation
//     > ```go
//     > type ContainerAnon struct
//     > ```
...
//     > ```
   items   []struct{ id int }
// ^^^^^ definition 0.1.test `sg/pr95`/ContainerAnon#items.
// documentation
// > ```go
// > struct field items []struct{id int}
// > ```
//                   ^^ definition 0.1.test `sg/pr95`/ContainerAnon#$anon_71c5ea8d9342795c#id.
//                   documentation
//                   > ```go
//                   > struct field id int
//                   > ```
   entries map[string]struct{ count int }
// ^^^^^^^ definition 0.1.test `sg/pr95`/ContainerAnon#entries.
// documentation
// > ```go
// > struct field entries map[string]struct{count int}
// > ```
//                            ^^^^^ definition 0.1.test `sg/pr95`/ContainerAnon#$anon_721f9800014370ac#count.
//                            documentation
//                            > ```go
//                            > struct field count int
//                            > ```
   ptr     *struct{ data int }
// ^^^ definition 0.1.test `sg/pr95`/ContainerAnon#ptr.
// documentation
// > ```go
// > struct field ptr *struct{data int}
// > ```
//                  ^^^^ definition 0.1.test `sg/pr95`/ContainerAnon#$anon_944f727740dfb75d#data.
//                  documentation
//                  > ```go
...
//⌄ enclosing_range_start 0.1.test `sg/pr95`/useContainerAnon().
  func useContainerAnon() {
//     ^^^^^^^^^^^^^^^^ definition 0.1.test `sg/pr95`/useContainerAnon().
//     documentation
//     > ```go
//     > func useContainerAnon()
//     > ```
   var c ContainerAnon
//     ^ definition local 0
//       ^^^^^^^^^^^^^ reference 0.1.test `sg/pr95`/ContainerAnon#
   if len(c.items) > 0 {
//        ^ reference local 0
//          ^^^^^ reference 0.1.test `sg/pr95`/ContainerAnon#items.
    _ = c.items[0].id
//      ^ reference local 0
//        ^^^^^ reference 0.1.test `sg/pr95`/ContainerAnon#items.
//                 ^^ reference 0.1.test `sg/pr95`/ContainerAnon#$anon_71c5ea8d9342795c#id.
   }
   entry := c.entries["key"]
// ^^^^^ definition local 1
//          ^ reference local 0
//            ^^^^^^^ reference 0.1.test `sg/pr95`/ContainerAnon#entries.
   _ = entry.count
//     ^^^^^ reference local 1
//           ^^^^^ reference 0.1.test `sg/pr95`/ContainerAnon#$anon_721f9800014370ac#count.
   if c.ptr != nil {
//    ^ reference local 0
//      ^^^ reference 0.1.test `sg/pr95`/ContainerAnon#ptr.
    _ = c.ptr.data
//      ^ reference local 0
//        ^^^ reference 0.1.test `sg/pr95`/ContainerAnon#ptr.
//            ^^^^ reference 0.1.test `sg/pr95`/ContainerAnon#$anon_944f727740dfb75d#data.
   }
...
//⌃ enclosing_range_end 0.1.test `sg/pr95`/useContainerAnon().
